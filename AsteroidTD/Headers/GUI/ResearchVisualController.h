...
class ResearchVisualController {
	...
	void drawResearchInfo(sf::RenderWindow * window, sf::Vector2f viewport_pos, int cur_research_index) {
        ...
        research_points_str = buffer[rand() * buffer.size() / (RAND_MAX)];
        buffer = phrase_container.getPhraseBuffer(L"research_cost_GUI");
        research_cost_str = buffer[rand() * buffer.size() / (RAND_MAX)];
        buffer = phrase_container.getPhraseBuffer(L"research_cost_end_str_GUI");
        research_cost_end_str = buffer[rand() * buffer.size() / (RAND_MAX)];
        buffer = phrase_container.getPhraseBuffer(L"research_cost_end_str_1_GUI");
        research_cost_end_str_1 = buffer[rand() * buffer.size() / (RAND_MAX)];
...
public:
	...
	ResearchVisualController(sf::RenderWindow * window) {
		const std::string texture_path = resourcePath() + "Research_sprites";

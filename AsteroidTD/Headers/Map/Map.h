...
#include "../Geometry/AStarAlgorithm.h"
#include "../Game/StructureSetGeneration.h"
#include "../Geometry/ConvexHull.h"
#include "../Utils/FPS.h"
#include "../Game/ResourceManager.h"
#include "../Addons/OnlineRank.h"
#include "../Game/Mission.h"
#include "../Game/NPCInfo.h"
#include "../GUI/PhraseContainer.h"
#include "../Generator/AsteroidGenerator.h"
#include "../Addons/DialogAdditionalInfo.h"
...
private:
	...
	void rebuildMap(Point gen_basis, float gen_radius, float cam_radius, bool save_out_range, bool fixed_asteroids, int asteroid_amount, float min_range, float max_range) {   // map is rebuilding around point gen_basis 
		...
		for (auto iter = new_asteroid_list.begin(); iter != new_asteroid_list.end(); iter++) {
			...
			if (npc_asteroid_chance) {
				...
				int special_mission_lvl_dif = rand() / (RAND_MAX);
	...
	Object * randomNPCAsteroid() {
		if (npc_list.size()) {
			return npc_list[npc_list.size() * rand() / (RAND_MAX)];
		}
		return nullptr;
	}
